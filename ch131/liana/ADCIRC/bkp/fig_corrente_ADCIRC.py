...
if sys.argv[4] == "curr":
	...
	for dd in range(0,numdays):
	   ...
	   lat_sul   = np.nanmin(lat) # comentar e alterar no início, caso queira um ponto específico
	   lat_norte = np.nanmax(lat) # comentar e alterar no início, caso queira um ponto específico
	   lon_oeste = np.nanmin(lon) # comentar e alterar no início, caso queira um ponto específico
	   lon_leste = np.nanmax(lon) # comentar e alterar no início, caso queira um ponto específico
	   lat_s  = float(lat_sul)
	...
	#      plt.savefig('/home/operador/AuxDec_HYCOM/corrente_hycom/Fig_HYCOM_manual/Figuras_ADCIRC/ADCIRC_'+nome+'_nos_'+dt+'_'+tit_horas[ttt]+'Z.png')
		   plt.savefig('/home/pyusers/ch131/liana/ADCIRC/figuras_ADCIRC/correntes/ADCIRC_'+nome+'_nos_'+dt+'_'+tit_horas[ttt]+'Z.png')
...
## ALTURA SIGNIFICATIVA
	...
	for dd in range(0,numdays):
	   ...
	   # ARquivo de Hs
	   DIR    = '/mnt/nfs/dpns32/data1/operador/Aplicativos_REMO/ADCIRC/adcirc_novacomp/adcircswan_BG_operacional/'+dt+'/'
	   nc_hsf = DIR+'ADCIRC_BG_hs_'+dt+'_graderegular.nc'
	   nc_fid = Dataset(nc_hsf, 'r')
	   lat    = nc_fid.variables['lat'][:]
	   lon    = nc_fid.variables['lon'][:]
	   lat_sul   = np.nanmin(lat) # comentar e alterar no início, caso queira um ponto específico
	   lat_norte = np.nanmax(lat) # comentar e alterar no início, caso queira um ponto específico
	   ...
	   DIR     = '/mnt/nfs/dpns32/data1/operador/Aplicativos_REMO/ADCIRC/adcirc_novacomp/adcircswan_BG_operacional/'+dt+'/'
	   nc_dirf = DIR+'ADCIRC_BG_dir_'+dt+'_graderegular.nc'
	   nc_fid  = Dataset(nc_dirf, 'r')
	   wdir    = nc_fid.variables['dir'][0:48,:,:] # graus
	   wdir    = np.squeeze(wdir)
	   wdir0   = np.copy(wdir)  
	   tmax    = np.size(wdir[:,0,0])
	   u       = np.cos(wdir)*3.14/180
	   v       = np.sin(wdir)*3.14/180
	   u       = np.squeeze(u); v  = np.squeeze(v)
	   u0      = np.copy(u);    v0 = np.copy(v)
	   ...
	   for ii in range(0,tmax,12):
		   ...
		   m     = Basemap(projection='merc', llcrnrlat=lat_s, urcrnrlat=lat_n, llcrnrlon = float(lon_oeste), urcrnrlon=float(lon_leste), lat_ts=float(lat_media), resolution='h')
		   x,y   = m(lon,lat)
		   cores = np.linspace(0,4,num=13)
		   CS    = m.contourf(x,y,hss,levels=cores,cmap=plt.cm.Blues)
		   ...
		   # Incluindo os vetores de direção da onda na figura
		   Q    = m.quiver(x[::skip, ::skip], y[::skip, ::skip], 150*uu[::skip, ::skip], 150*vv[::skip, ::skip], scale=30, units='inches', scale_units='inches', width=0.008, headlength=5, headwidth=3.5, headaxislength=5, pivot='tail', color='k', minshaft=2, minlength=1)
		   plt.title('ADCIRC CHM/REMO '+dt+' '+tit_horas[ttt]+'Z') 
		   cbar = plt.colorbar(CS, format='%.2f')
		   cbar.ax.set_ylabel('altura significativa [m]')
	#      plt.savefig('/home/operador/AuxDec_HYCOM/corrente_hycom/Fig_HYCOM_manual/Figuras_ADCIRC/ADCIRC_'+nome+'_Hs_'+dt+'_'+tit_horas[ttt]+'Z.png')
		   plt.savefig('/home/pyusers/ch131/liana/ADCIRC/figuras_ADCIRC/hs/ADCIRC_'+nome+'_'+dt+'_'+tit_horas[ttt]+'Z.png')
